...
export default function Dashboard() {
  ...
  const {
    data: realTrueLayerData,
    loading: trueLayerLoading,
      ...
      if (token) {
        ...
        try {
          ...
          const accountsWithTransactions =
            await fetchAccountsWithTransactions(token);

          console.log(
            "✅ TrueLayer data fetched successfully:",
  ...
  const dashboardData = {
    user: user
      ...
      : {
          ...dummyUser,
          name: dummyUser.displayName || dummyUser.firstName || "User",
        },
          ...
          {trueLayerData && (
            <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg">
              <h3 className="text-green-800 font-medium mb-3">
                🏦 TrueLayer Bank Data Loaded Successfully
              </h3>
                    ...
                    {account.account_number && (
                      <div className="text-xs text-gray-500 mt-1">
                        {account.account_number.sort_code &&
                          `Sort: ${account.account_number.sort_code}`}
                        {account.account_number.number &&
                          ` | Account: ${account.account_number.number}`}
                      </div>
                    )}
                  </div>
                ))}
              ...
              <p className="text-sm text-green-600 mt-3">
                ✅ Check browser console for detailed account and transaction
                data
              </p>
            </div>
          )}
